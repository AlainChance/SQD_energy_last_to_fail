...
\toprule
state & $\langle r^{2}\rangle$ & $\Theta_{xx}$ & $\rho(\mathrm{N})$ & $\langle r^{2}\rangle$ error (\%) & $\Theta_{xx}$ error (\%) & $\rho(\mathrm{N})$ error (\%) \\
\midrule
...
\toprule
samples & $\Delta E$ (mHa) & $\langle r^{2}\rangle$ error (\%) & $\Theta_{xx}$ error (\%) & $\rho(\mathrm{N})$ error (\%) \\
\midrule
...
\begin{table}[htbp]
\centering
\caption{Exact samples of the LUCJ circuit and of the time-evolved state in the (10e, 12o) window of N$_2$ at 1.0975 \AA{}, 50 000 shots each, and the static loop on them; full CI $-$108.999490 Ha, Hartree--Fock $-$108.867774 Ha. The hardware pattern keeps the alpha--alpha interactions between neighbouring orbitals and the alpha--beta ones on the same orbital, as the heavy-hex layout does; ``parameters optimized'' means the circuit's parameters minimized against the exact energy of the window (linear method, fifteen iterations) instead of being read from the coupled-cluster amplitudes.}\label{tab:10}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\linewidth}{@{}>{\hsize=1.279\hsize\raggedright\arraybackslash}Xlll>{\hsize=0.721\hsize\raggedright\arraybackslash}X@{}}
\toprule
state sampled & distinct configurations & $\alpha$ strings & full-CI weight captured & static loop above full CI (mHa) \\
\midrule
LUCJ, one layer, from CCSD amplitudes & 37 & 16 & 0.977 & 30.63 \\
LUCJ, one layer, hardware pattern, from CCSD amplitudes & 18 & 10 & 0.958 & 43.14 \\
LUCJ, one layer, hardware pattern, parameters optimized & 204 & 62 & 0.987 & 7.81 \\
LUCJ, one layer, all-to-all, parameters optimized & 206 & 65 & 0.992 & 3.90 \\
evolved HF, t = 0.25 & 143 & 53 & 0.994 & 5.10 \\
evolved HF, t = 0.5 & 274 & 86 & 0.998 & 1.83 \\
evolved HF, t = 1.0 & 547 & 157 & 0.999 & 0.78 \\
evolved HF, t = 2.0 & 631 & 162 & 0.998 & 0.58 \\
evolved HF, t = 4.0 & 756 & 213 & 0.998 & 0.43 \\
evolved HF, t = 8.0 & 832 & 229 & 0.998 & 0.48 \\
\bottomrule
\end{tabularx}
\end{table}
...
The circuit's parameters are a variable the demonstrations do not use, and Table 10 measures it. The circuits sampled on hardware take their parameters from the coupled-cluster amplitudes without optimization, and the configuration prior of \S\ref{sec:3.4} is not that input but coupled cluster's output, the largest determinants of its wavefunction; the 2.3 mHa of Table 11 is what a sampled one-layer circuit loses against the amplitudes that parametrized it. Optimizing the parameters is the missing rung between the 142 and the 2 146 configurations of Table 8. In the window, the hardware pattern read from the amplitudes holds 18 configurations and its static loop stops 43 mHa short; optimized against the exact energy it holds 204, captures 0.987 of the full-CI weight and stops 7.8 mHa short; the all-to-all circuit goes from 37 configurations and 30.6 mHa to 206 and 3.9 mHa. Optimization broadens a one-layer circuit's support by an order of magnitude and leaves it a factor of four below the evolved state's 756 configurations, with a static-loop error nine to eighteen times the evolved state's; the circuit's energy improves by 64 and 52 mHa along the way. The ordering of Table 10 is therefore one of circuit families, not of parameters: what the loop needs is support, and a one-layer cluster-Jastrow circuit, however parametrized, concentrates its samples on a few hundred configurations.
