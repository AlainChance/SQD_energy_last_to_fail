...
\begin{table}[htbp]
\centering
\caption{Exact samples of the LUCJ circuit and of the time-evolved state in the (10e, 12o) window of N$_2$ at 1.0975 \AA{}, 50 000 shots each, and the static loop on them; full CI $-$108.999490 Ha, Hartree--Fock $-$108.867774 Ha. The hardware pattern keeps the alpha--alpha interactions between neighbouring orbitals and the alpha--beta ones on the same orbital, as the heavy-hex layout does; ``parameters optimized to convergence'' means the circuit's parameters minimized against the exact energy of the window (linear method, run to its convergence test of a $10^{-8}$ relative reduction per step) instead of being read from the coupled-cluster amplitudes; those two rows give ranges over independent optimizations, three of each circuit from the same start and a fourth of the all-to-all circuit from a perturbed start, because a fifteen-iteration snapshot of either was not reproducible.}\label{tab:10}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\linewidth}{@{}>{\hsize=1.386\hsize\raggedright\arraybackslash}Xlll>{\hsize=0.614\hsize\raggedright\arraybackslash}X@{}}
\toprule
...
LUCJ, one layer, hardware pattern, parameters optimized to convergence & 233--269 & 68--70 & 0.987--0.989 & 6.0--7.6 \\
LUCJ, one layer, all-to-all, parameters optimized to convergence & 177--256 & 47--74 & 0.988--0.997 & 2.3--16.4 \\
...
\begin{tabular}{@{}lllllll@{}}
\toprule
subspace & strings per spin & D & seconds & s per 1$0^{6}$ & peak GB & $\Delta$ vs recorded (Ha) \\
\midrule
